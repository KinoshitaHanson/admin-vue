    ...
    <div class="form-container">
      <el-form :inline="true" :model="form" class="demo-form-inline">
        <el-form-item label="Id">
          <el-input v-model="form.id" placeholder="美图Id"></el-input>
        </el-form-item>
...
<script>
import { RobotPictureSelect, RobotPictureUpdate, RobotPictureCategorySelect } from 'api';
...
export default {
  ...
  methods: {
    ...
    async getData(pageIndex, pageSize) {
      ...
      try {
        let res = await RobotPictureSelect(params);
        if (res.code == 0) {
    ...
    async toggleStatus(o) {
      ...
      try {
        let res = await RobotPictureUpdate(data);
        if (res.code == 0) {
    ...
    async initCategory() {
      try {
        let res = await RobotPictureCategorySelect({});
        if (res.code == 0) {
