...
<script>
import { RobotPictureSelectOne, RobotPictureAdd, RobotPictureUpdate, RobotPictureCategorySelect } from 'api';
...
export default {
  ...
  methods: {
    ...
    async getData() {
      ...
      try {
        let res = await RobotPictureSelectOne(params);
    ...
    async onSubmit() {
      ...
      try {
        let res;
        if (data.id) {
          res = await RobotPictureUpdate(data);
        } else {
          res = await RobotPictureAdd(data);
        }
        if (res.code == 0) {
